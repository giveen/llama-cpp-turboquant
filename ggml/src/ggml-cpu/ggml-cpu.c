...
static void ggml_vec_dot_q8_cr_f32(int                        n,
                                   float * GGML_RESTRICT      s,
                                   size_t                     bs,
                                   const void * GGML_RESTRICT vx,
                                   size_t                     bx,
...
static void ggml_vec_dot_q5_cr_f32(int                        n,
                                   float * GGML_RESTRICT      s,
                                   size_t                     bs,
                                   const void * GGML_RESTRICT vx,
                                   size_t                     bx,
...
static void ggml_vec_dot_q6_cr_f32(int                        n,
                                   float * GGML_RESTRICT      s,
                                   size_t                     bs,
                                   const void * GGML_RESTRICT vx,
                                   size_t                     bx,
                                   ...
                                   size_t                     by,
                                   int                        nrc);
static void ggml_vec_dot_tq3_1s_q8_0(int                        n,
                                     float * GGML_RESTRICT      s,
                                     size_t                     bs,
                                     const void * GGML_RESTRICT vx,
                                     size_t                     bx,
                                     ...
                                     size_t                     by,
                                     int                        nrc);
static void ggml_vec_dot_tq4_1s_q8_0(int                        n,
                                     float * GGML_RESTRICT      s,
                                     size_t                     bs,
                                     const void * GGML_RESTRICT vx,
                                     size_t                     bx,
...
struct ggml_threadpool {
    ...
    atomic_int GGML_CACHE_ALIGN
        current_chunk;  // currently processing chunk during Mat_Mul, shared between all the threads.
        ...
        for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
            for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ir1 += num_rows_per_vec_dot) {
                ...
                for (int64_t ir0 = iir0; ir0 < iir0 + blck_0 && ir0 < ir0_end; ir0 += num_rows_per_vec_dot) {
                    vec_dot(ne00, &tmp[ir0 - iir0], (num_rows_per_vec_dot > 1 ? 16 : 0), src0_row + ir0 * nb01,
                            (num_rows_per_vec_dot > 1 ? nb01 : 0), src1_col,
...
static void ggml_compute_forward_mul_mat_id_one_chunk(struct ggml_tensor *            dst,
    ...
    for (int64_t iir1 = ir1_start; iir1 < ir1_end; iir1 += blck_1) {
        for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
            for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ++ir1) {
                ...
                // TODO: this is a bit of a hack, we should probably have a better way to handle this
                const char * src1_col =
                    (const char *) wdata + (src1_cont || src1->type != vec_dot_type ? (i11 + i12 * ne11) * row_size :
                                                                                      (i11 * nb11 + i12 * nb12));
...
static void ggml_compute_forward_mul_mat_id_impl(const struct ggml_compute_params * params,
                                                 struct ggml_tensor *               dst,
                                                 uint64_t                           row_mask,
                                                 bool                               use_row_mask,
    ...
    const int64_t moe_cache_n_rows =
        ids->ne[0] > 0 && ids->ne[1] > 0 && ids->ne[0] <= INT64_MAX / ids->ne[1] ? ids->ne[0] * ids->ne[1] : INT64_MAX;
    const bool    moe_cache_rows_fit = moe_cache_n_rows <= MOE_CACHE_MAX_TOPK;
    ...
    if (ith == 0) {
        ...
        if (allow_moe_cache && moe_cache.begin && moe_cache.plan && moe_cache.dispatch && moe_cache.collect &&
            moe_cache.end && src0->op == GGML_OP_NONE && src0_buffer && ggml_backend_buffer_is_host(src0_buffer) &&
            ggml_backend_buffer_get_usage(src0_buffer) == GGML_BACKEND_BUFFER_USAGE_WEIGHTS &&
            src1->type == GGML_TYPE_F32) {
            moe_cache_node = moe_cache.begin(src0->name, src0->data, nb02, ne00, ne01, (int) type, ne02, ids->ne[1],
                                             moe_cache_n_rows);
        ...
        for (int64_t iid1 = 0; iid1 < ids->ne[1]; ++iid1) {
            for (int id = 0; id < n_ids; ++id) {
                ...
                if (moe_cache_node && moe_cache_slot_idx[iid1 * n_ids + id] >= 0) {
                    ...
                    moe_cache_acts[moe_cache_n_hits] =
                        (const float *) ((const char *) src1->data + i11 * nb11 + iid1 * nb12);
        ...
        if (moe_cache_node && moe_cache_n_hits > 0) {
            if (!moe_cache.dispatch(moe_cache_node, (int) type, ne00, ne01, moe_cache_n_hits, moe_cache_compact,
                                    moe_cache_acts)) {
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
        ...
        while (current_chunk < nchunk0 * nchunk1) {
            ...
            ggml_compute_forward_mul_mat_id_one_chunk(dst, src0, src1, ids, cur_a, ir0_start, ir0_end, ir1_start,
                                                      ir1_end, src0_cur, matrix_rows, row_size, src1_cont, wdata);
    ...
    if (ith == 0 && moe_cache_node) {
        ...
        if (!moe_cache.collect(moe_cache_node, moe_cache_n_hits, moe_cache_rows, ne0)) {
            ...
            for (int i = 0; i < moe_cache_n_hits; i++) {
                ...
                ggml_compute_forward_mul_mat_id_one_chunk(dst, src0, src1, ids, expert, 0, ne01, row, row + 1,
                                                          (const char *) src0->data + expert * nb02, matrix_rows,
                                                          row_size, src1_cont, wdata);
            }
        }
        moe_cache.end(moe_cache_node);
    }
...
static void ggml_compute_forward_mul_mat_id(const struct ggml_compute_params * params, struct ggml_tensor * dst) {
                ...
                case GGML_OP_DUP:
                    {
                        if (ggml_is_quantized(node->type) ||
                            ...
                            (node->src[0]->type == GGML_TYPE_F16 && node->src[1] &&
                             node->src[1]->type == GGML_TYPE_BF16) ||
                            (node->src[0]->type == GGML_TYPE_BF16 && node->src[1] &&
                             node->src[1]->type == GGML_TYPE_F16) ||
                            // conversion between F32 and I32
                            (node->src[0]->type == GGML_TYPE_F32 && node->src[1] &&
                             node->src[1]->type == GGML_TYPE_I32) ||
                            (node->src[0]->type == GGML_TYPE_I32 && node->src[1] &&
                ...
                case GGML_OP_CONV_TRANSPOSE_1D:
                    {
                        ...
                        if ((node->src[0]->type == GGML_TYPE_F16 || node->src[0]->type == GGML_TYPE_BF16) &&
                            node->src[1]->type == GGML_TYPE_F32) {
                ...
                case GGML_OP_FLASH_ATTN_EXT:
                    {
                        ...
                        size_t prefill = sizeof(float) *
                                         (GGML_FA_TILE_Q * DK + 2 * GGML_FA_TILE_Q * GGML_FA_TILE_KV +
                ...
                case GGML_OP_FLASH_ATTN_BACK:
                    {
                        ...
                        const int64_t mxDn =
                            MAX(D, ne11) * 2;  // *2 because of S and SM in ggml_compute_forward_flash_attn_back
...
static bool ggml_moe_cache_weight_is_eligible(const struct ggml_tensor * weight) {
    if (!weight || weight->op != GGML_OP_NONE || !weight->data || weight->ne[0] <= 0 || weight->ne[1] <= 0 ||
        weight->ne[2] <= 0 || weight->nb[0] != ggml_type_size(weight->type)) {
        return false;
    }
    ggml_backend_buffer_t buffer = weight->view_src ? weight->view_src->buffer : weight->buffer;
...
static bool ggml_moe_cache_can_fuse(const struct ggml_cgraph *     cgraph,
                                    int                            node_n,
                                    struct ggml_moe_cache_fusion * fusion) {
    const struct ggml_moe_cache_api moe_cache = ggml_moe_cache_active();
    if (!moe_cache.fused_begin || !moe_cache.collect || !moe_cache.end) {
        return false;
    }
    ...
    if (node_n + 4 < cgraph->n_nodes) {
        ...
        const enum ggml_op grouped[] = {
            GGML_OP_MUL_MAT_ID, GGML_OP_MUL_MAT_ID, GGML_OP_CLAMP, GGML_OP_CLAMP, GGML_OP_GLU,
        };
        ...
        if (ggml_can_fuse_subgraph(cgraph, node_n, 5, interleaved, &output, 1) ||
            ...
            if (ggml_get_glu_op(result) == GGML_GLU_OP_SWIGLU && gate_clamp && up_clamp && gate_clamp != up_clamp &&
                gate_clamp->op == GGML_OP_CLAMP && up_clamp->op == GGML_OP_CLAMP && gate_clamp->src[0] &&
                up_clamp->src[0] && gate_clamp->src[0] != up_clamp->src[0] &&
                gate_clamp->src[0]->op == GGML_OP_MUL_MAT_ID && up_clamp->src[0]->op == GGML_OP_MUL_MAT_ID) {
    ...
    if (!fusion->glu && node_n + 2 < cgraph->n_nodes) {
        ...
        if (ggml_can_fuse_subgraph(cgraph, node_n, 3, ops, &output, 1)) {
            ...
            struct ggml_tensor * result = cgraph->nodes[output];
            if (ggml_get_glu_op(result) == GGML_GLU_OP_SWIGLU && result->src[0] && result->src[1] &&
    ...
    if (!fusion->up || !fusion->gate || !fusion->glu || isnan(fusion->up_min) || isnan(fusion->up_max) ||
        isnan(fusion->gate_min) || isnan(fusion->gate_max) || fusion->up_min > fusion->up_max ||
    ...
    if (!ggml_moe_cache_weight_is_eligible(up_weight) || !ggml_moe_cache_weight_is_eligible(gate_weight) || !acts ||
        !ids || acts != fusion->gate->src[1] || ids != fusion->gate->src[2] || acts->type != GGML_TYPE_F32 ||
        ids->type != GGML_TYPE_I32 || ids->ne[0] < 1 || ids->ne[1] < 1 || ids->ne[0] > MOE_CACHE_MAX_TOPK ||
        ids->ne[1] > MOE_CACHE_MAX_TOPK / ids->ne[0] || acts->ne[1] < 1 || acts->ne[2] != ids->ne[1] ||
        up_weight->type != gate_weight->type || up_weight->ne[0] != gate_weight->ne[0] ||
        up_weight->ne[1] != gate_weight->ne[1] || up_weight->ne[2] != gate_weight->ne[2] ||
        up_weight->nb[2] != gate_weight->nb[2] || fusion->up->type != GGML_TYPE_F32 ||
        fusion->gate->type != GGML_TYPE_F32 || fusion->glu->type != GGML_TYPE_F32 ||
        !ggml_are_same_shape(fusion->up, fusion->gate) || !ggml_are_same_shape(fusion->up, fusion->glu) ||
        !ggml_is_contiguous_1(fusion->up) || !ggml_is_contiguous_1(fusion->gate) ||
        !ggml_is_contiguous_1(fusion->glu) || fusion->up->ne[1] != ids->ne[0] || fusion->up->ne[2] != ids->ne[1] ||
        ggml_nrows(fusion->up) != ids->ne[0] * ids->ne[1] || up_weight->ne[0] != acts->ne[0] ||
        up_weight->ne[1] != fusion->up->ne[0] || up_weight->ne[2] <= 0) {
        return false;
    }
    return true;
}

static void ggml_compute_forward_swiglu_masked(const struct ggml_compute_params * params,
    ...
    const int     worker_count = reserve_thread_zero && params->nth > 1 ? params->nth - 1 : params->nth;
    const int     worker       = reserve_thread_zero && params->nth > 1 ? params->ith - 1 : params->ith;
    if (worker < 0) {
        return;
    }
    ...
    for (int64_t row = worker; row < n_rows; row += worker_count) {
        ...
        if (!clamped) {
            ggml_vec_swiglu_f32((int) dst->ne[0], dst_row, gate_row, up_row);
            continue;
        }
        for (int64_t col = 0; col < dst->ne[0]; col++) {
            const float gate_value = MAX(MIN(gate_row[col], gate_max), gate_min);
            const float up_value   = MAX(MIN(up_row[col], up_max), up_min);
            dst_row[col]           = ggml_silu_f32(gate_value) * up_value;
        }
    }
}

static int ggml_cpu_try_fuse_moe_cache(const struct ggml_cgraph *         cgraph,
                                       int                                node_n,
                                       const struct ggml_compute_params * params) {
    struct ggml_moe_cache_fusion fusion;
    if (!ggml_moe_cache_can_fuse(cgraph, node_n, &fusion) || !params->wdata ||
        params->wsize < MOE_CACHE_FUSED_WORK_SIZE) {
        return 0;
    }
    const struct ggml_moe_cache_api moe_cache = ggml_moe_cache_active();
    ...
    struct moe_cache_fused_state * state       = (struct moe_cache_fused_state *) params->wdata;
    ...
    if (params->ith == 0) {
        ...
        struct ggml_moe_cache_tensor_desc up_desc = {
            up_weight->name,  up_weight->data,  up_weight->nb[2],          up_weight->ne[0],
            up_weight->ne[1], up_weight->ne[2], (int32_t) up_weight->type,
        };
        struct ggml_moe_cache_tensor_desc gate_desc = {
            gate_weight->name,  gate_weight->data,  gate_weight->nb[2],          gate_weight->ne[0],
            gate_weight->ne[1], gate_weight->ne[2], (int32_t) gate_weight->type,
        };
        for (int token = 0; token < n_tokens; token++) {
            for (int id = 0; id < n_ids; id++) {
                const int row    = token * n_ids + id;
                state->ids[row]  = *(const int32_t *) ((const char *) ids->data + token * ids->nb[1] + id * ids->nb[0]);
                state->acts[row] = (const float *) ((const char *) acts->data + token * acts->nb[2] +
                                                    (id % acts->ne[1]) * acts->nb[1]);
            }
        }
        state->node = moe_cache.fused_begin(&up_desc, &gate_desc, (int) GGML_GLU_OP_SWIGLU, fusion.up_min,
                                            fusion.up_max, fusion.gate_min, fusion.gate_max, state->ids, n_rows,
                                            n_tokens, state->acts, &state->hit_mask);
        if (state->node) {
            for (int row = 0; row < n_rows; row++) {
                if (state->hit_mask & (UINT64_C(1) << row)) {
                    state->rows[state->n_hits++] = (float *) ((char *) glu->data + row * glu->nb[1]);
                }
            }
        }
    }
    ...
    const uint64_t valid_mask             = n_rows == MOE_CACHE_MAX_TOPK ? UINT64_MAX : (UINT64_C(1) << n_rows) - 1;
    const uint64_t miss_mask              = valid_mask & ~state->hit_mask;

    ggml_compute_forward_mul_mat_id_impl(&sub_params, up, miss_mask, true, false);
    ggml_barrier(params->threadpool);
    ggml_compute_forward_mul_mat_id_impl(&sub_params, gate, miss_mask, true, false);
    ggml_barrier(params->threadpool);

    ggml_compute_forward_swiglu_masked(params, gate, up, glu, miss_mask, true, fusion.clamped, fusion.up_min,
                                       fusion.up_max, fusion.gate_min, fusion.gate_max);
    if (params->ith == 0) {
        state->collect_ok = moe_cache.collect(state->node, state->n_hits, state->rows, glu->ne[0]);
    ...
    if (!state->collect_ok) {
        ggml_compute_forward_mul_mat_id_impl(&sub_params, up, state->hit_mask, true, false);
        ggml_barrier(params->threadpool);
        ggml_compute_forward_mul_mat_id_impl(&sub_params, gate, state->hit_mask, true, false);
        ggml_barrier(params->threadpool);
        ggml_compute_forward_swiglu_masked(params, gate, up, glu, state->hit_mask, false, fusion.clamped, fusion.up_min,
                                           fusion.up_max, fusion.gate_min, fusion.gate_max);
    }
...
static int ggml_cpu_try_fuse_ops(const struct ggml_cgraph *         cgraph,
                                 const int                          node_n,
                                 const struct ggml_compute_params * params,
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n;
         node_n++) {
        struct ggml_tensor * node = cgraph->nodes[node_n];
...
static inline bool ggml_graph_compute_thread_ready(struct ggml_compute_state * state) {
    ...
    if (state->pending || threadpool->stop || threadpool->pause) {
        return true;
    }
...
static struct ggml_threadpool * ggml_threadpool_new_impl(struct ggml_threadpool_params * tpp,
                                                         struct ggml_cgraph *            cgraph,
                                                         struct ggml_cplan *             cplan) {
...
static void ggml_vec_dot_tq_q8_0_impl(enum ggml_type             type_x,
                                      int                        n,
                                      float * GGML_RESTRICT      s,
                                      const void * GGML_RESTRICT vx,
                                      const void * GGML_RESTRICT vy) {
...
static void ggml_vec_dot_cr_f32_impl(enum ggml_type             type_x,
                                     int                        n,
                                     float * GGML_RESTRICT      s,
                                     const void * GGML_RESTRICT vx,
                                     const void * GGML_RESTRICT vy) {
...
#    endif
    for (; i + 7 < n; i += 8) {
        _mm256_storeu_ps(y + i, _mm256_castsi256_ps(_mm256_slli_epi32(
                                    _mm256_cvtepu16_epi32(_mm_loadu_si128((const __m128i *) (x + i))), 16)));
    }
