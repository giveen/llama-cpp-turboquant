...
static __device__ __forceinline__ float convrot_inverse_group(float buf[2][QK8_CR], const int tid) {
    __syncthreads();
    float v = buf[0][tid];
...
static __global__ void mul_mat_vec_cr(const void * __restrict__ vx, const float * __restrict__ x,
        float * __restrict__ y, const int64_t k) {
    const int64_t row = blockIdx.x;
    const int tid = threadIdx.x;
    const int blocks_per_row = k/QK8_CR;
    float acc = 0.0f;
    ...
    for (int block = 0; block < blocks_per_row; ++block) {
        if (tid < QK8_CR/2) {
            const int i00 = 2*tid;
            const int ib = (row*blocks_per_row + block)*(QK8_CR/qk) + i00/qk;
            const int iqs = (i00%qk)/qr;
            const int iybs = i00 - i00%qk;
            const int y_offset = qr == 1 ? 1 : qk/2;
            float2 v;
...
static __global__ void mul_mat_vec_q6_cr(const void * __restrict__ vx, const float * __restrict__ x,
        float * __restrict__ y, const int64_t k) {
    const int64_t row = blockIdx.x;
    const int tid = threadIdx.x;
    const int blocks_per_row = k/QK8_CR;
...
void ggml_cuda_mul_mat_vec_cr(ggml_type type, const void * weights, const float * x, float * y,
    ...
    GGML_ASSERT(k % QK8_CR == 0);
    switch (type) {
        case GGML_TYPE_Q5_CR:
            mul_mat_vec_cr<QK5_0, QR5_0, dequantize_q5_0><<<m, QK8_CR, 0, stream>>>(weights, x, y, k);
            break;
        case GGML_TYPE_Q6_CR:
            mul_mat_vec_q6_cr<<<m, QK8_CR, 0, stream>>>(weights, x, y, k);
            break;
        case GGML_TYPE_Q8_CR:
            mul_mat_vec_cr<QK8_0, QR8_0, dequantize_q8_0><<<m, QK8_CR, 0, stream>>>(weights, x, y, k);
            break;
        default:
