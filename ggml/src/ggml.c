...
enum ggml_type ggml_ftype_to_ggml_type(enum ggml_ftype ftype) {
    ...
    switch (ftype) {
        ...
        case GGML_FTYPE_MOSTLY_Q1_0:
            wtype = GGML_TYPE_Q1_0;
            break;
        case GGML_FTYPE_MOSTLY_Q2_0:
            wtype = GGML_TYPE_Q2_0;
            break;
        ...
        case GGML_FTYPE_MOSTLY_Q8_0:
            ...
            break;
        case GGML_FTYPE_MOSTLY_Q8_CR:
            wtype = GGML_TYPE_Q8_CR;
            break;
        case GGML_FTYPE_MOSTLY_Q5_CR:
            wtype = GGML_TYPE_Q5_CR;
            break;
        case GGML_FTYPE_MOSTLY_Q6_CR:
            wtype = GGML_TYPE_Q6_CR;
            break;
...
struct ggml_tensor * ggml_conv_1d(struct ggml_context * ctx,
    ...
    struct ggml_tensor * im2col =
        ggml_im2col(ctx, a, b, s0, 0, p0, 0, d0, 0, false,
...
struct ggml_tensor * ggml_conv_1d_dw(struct ggml_context * ctx,
    ...
    struct ggml_tensor * im2col = ggml_im2col(ctx, a, new_b, s0, 0, p0, 0, d0, 0, false,
...
struct ggml_tensor * ggml_conv_2d(struct ggml_context * ctx,
                                  ...
                                  int                   d1) {
    struct ggml_tensor * im2col =
        ggml_im2col(ctx, a, b, s0, s1, p0, p1, d0, d1, true,
                    a->type == GGML_TYPE_BF16 ? GGML_TYPE_F32 : GGML_TYPE_F16);  // [N, OH, OW, IC * KH * KW]
...
struct ggml_tensor * ggml_conv_3d(struct ggml_context * ctx,
    ...
    struct ggml_tensor * im2col =
        ggml_im2col_3d(ctx, a, b, IC, s0, s1, s2, p0, p1, p2, d0, d1, d2,
                       a->type == GGML_TYPE_BF16 ? GGML_TYPE_F32 : GGML_TYPE_F16);  // [N*OD, OH, OW, IC * KD * KH * KW]
...
struct ggml_tensor * ggml_conv_2d_dw(struct ggml_context * ctx,
    ...
    struct ggml_tensor * im2col =
        ggml_im2col(ctx, new_a, ggml_reshape_4d(ctx, b, b->ne[0], b->ne[1], 1, b->ne[2] * b->ne[3]), s0, s1, p0, p1, d0,
                    d1, true, a->type == GGML_TYPE_BF16 ? GGML_TYPE_F32 : GGML_TYPE_F16);  // [N * IC, OH, OW, KH * KW]
...
struct ggml_tensor * ggml_gated_delta_net(struct ggml_context * ctx,
    ...
    const int64_t state_rows =
        emit_mode == 0 ? K * S_v * n_seqs : K * 4 * n_seqs + S_v * n_seqs + (needs_ckpt ? S_v * n_seqs : 0);
...
struct ggml_tensor * ggml_kvarn_seal(struct ggml_context * ctx,
    ...
    struct ggml_tensor * result =
        ggml_new_tensor_3d(ctx, GGML_TYPE_I8, (int64_t) layout.tile_bytes, n_complete, n_head_kv);
...
struct ggml_tensor * ggml_kvarn_attn_decode(struct ggml_context * ctx,
...
struct ggml_tensor * ggml_lightning_indexer(struct ggml_context * ctx,
                                            struct ggml_tensor *  q,
                                            struct ggml_tensor *  k,
...
struct ggml_tensor * ggml_dsv4_hc_comb(struct ggml_context * ctx,
...
struct ggml_tensor * ggml_dsv4_hc_pre(struct ggml_context * ctx, struct ggml_tensor * x, struct ggml_tensor * weights) {
...
struct ggml_tensor * ggml_dsv4_hc_post(struct ggml_context * ctx,
                                       struct ggml_tensor *  x,
...
struct ggml_cgraph ggml_graph_view(struct ggml_cgraph * cgraph0, int i0, int i1) {
    struct ggml_cgraph cgraph = { /*.size             =*/0,
                                  /*.n_nodes          =*/i1 - i0,
                                  /*.n_leafs          =*/0,
...
size_t ggml_quantize_chunk(enum ggml_type type,
    ...
    switch (type) {
        case GGML_TYPE_Q1_0:
            result = quantize_q1_0(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_Q2_0:
            result = quantize_q2_0(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_Q4_0:
        ...
        case GGML_TYPE_Q8_CR:
            result = quantize_q8_cr(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_Q5_CR:
            result = quantize_q5_cr(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_Q6_CR:
            result = quantize_q6_cr(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_MXFP4:
            result = quantize_mxfp4(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_NVFP4:
            result = quantize_nvfp4(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_Q2_K:
        ...
        case GGML_TYPE_TQ3_1S:
            result = quantize_tq3_1s(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_TQ4_1S:
            result = quantize_tq4_1s(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix);
            break;
        case GGML_TYPE_F16:
            {
