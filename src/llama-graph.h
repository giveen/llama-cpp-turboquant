...
public:
    ...
    // OSCAR HP (F16 sink+recent) inputs - only valid when the kv cache has HP
    ggml_tensor * hp_k_idxs     = nullptr; // I64 [n_hp_batch]: global HP slot indices to write
    ggml_tensor * hp_batch_idxs = nullptr; // I32 [n_hp_batch]: which rows in k_cur are HP tokens
    ggml_tensor * hp_kq_mask    = nullptr; // F32 [n_hp_total, n_batch/n_stream, 1, n_stream]
    ggml_tensor * hp_kq_mask_cnv = nullptr; //     [n_hp_total, n_batch/n_stream, 1, n_stream]
...
public:
    ...
    // OSCAR HP (F16 sink+recent) inputs - per sub-cache (full + SWA)
    ggml_tensor * hp_k_idxs          = nullptr; // I64 [n_hp_batch]: global HP slot indices to write
    ggml_tensor * hp_batch_idxs      = nullptr; // I32 [n_hp_batch]: which rows in k_cur are HP tokens
    ggml_tensor * hp_kq_mask         = nullptr; // F32 [n_hp_total, n_batch/n_stream, 1, n_stream]
    ggml_tensor * hp_kq_mask_cnv     = nullptr; //     [n_hp_total, n_batch/n_stream, 1, n_stream]
