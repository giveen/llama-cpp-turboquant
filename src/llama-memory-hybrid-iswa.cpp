...
bool llama_memory_hybrid_iswa::seq_rm_plan(llama_seq_id seq_id, llama_pos p0, llama_pos p1, llama_pos * planned_p0, llama_pos * planned_p1) const {
    return llama_memory_seq_rm_plan_all(seq_id, p0, p1, { mem_attn.get(), mem_recr.get() }, planned_p0, planned_p1);
}

bool llama_memory_hybrid_iswa::state_seq_can_save(llama_seq_id seq_id) {
    return mem_attn->state_seq_can_save(seq_id) && mem_recr->state_seq_can_save(seq_id);
}

bool llama_memory_hybrid_iswa::state_seq_can_restore(llama_seq_id seq_id) {
